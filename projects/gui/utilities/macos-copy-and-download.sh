...
#
# Usage:
#   /path/to/macos-copy-and-download.sh /dev/ttyUSB0 /path/to/firmware.bin
...
DEFAULT_FIRMWARE_NAME="macos_0x18000000_V1.0.0.bin"
FIRMWARE_SUFFIX="0x18000000_V1.0.0.bin"
...
# The Ubuntu downloader auto-detects exactly one file with this suffix.
while IFS= read -r -d '' old_firmware; do
    rm -f "$old_firmware"
done < <(find "$PROJECT_DIR" -maxdepth 1 -type f -name "*${FIRMWARE_SUFFIX}" -print0)

target_file="${PROJECT_DIR}/${DEFAULT_FIRMWARE_NAME}"
cp "$staged_file" "$target_file"
rm -f "$staged_file"
trap - EXIT

echo "Copied firmware to shared project: $target_file"
exec "$REMOTE_DOWNLOADER" "$serial_port"
