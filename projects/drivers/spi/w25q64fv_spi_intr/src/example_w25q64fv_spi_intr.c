...
static int init(void)
{
    ...
    ret = csi_spi_attach_callback(&spi, spi_event_cb, NULL);
...
static int send(const void *data, uint32_t size)
{
    ...
    spi_event = -1;
    ret = csi_spi_send_async(&spi, data, size);
    ...
    while (spi_event != SPI_EVENT_SEND_COMPLETE) {
        if ((csi_tick_get() - timestart) > EXAMPLE_FLASH_TX_RX_TIMEOUT) {
            return -1;
        }
    }
...
static int receive(void *data, uint32_t size)
{
    ...
    while (spi_event != SPI_EVENT_RECEIVE_COMPLETE) {
        if ((csi_tick_get() - timestart) > EXAMPLE_FLASH_TX_RX_TIMEOUT) {
            return -1;
        }
    }
