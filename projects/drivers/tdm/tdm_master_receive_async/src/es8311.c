...
static inline int32_t es8311_write_reg(es8311_handle_t dev, uint8_t reg_addr, uint8_t data)
{
    ...
    return csi_iic_master_send(es->port, es->dev_addr, buff, 2, 100);
}
...
static inline int32_t es8311_read_reg(es8311_handle_t dev, uint8_t reg_addr, uint8_t *reg_value)
{
    ...
    csi_iic_master_send(es->port, es->dev_addr, &reg_addr, 1, 100);
    csi_iic_master_receive(es->port, es->dev_addr, reg_value, 1, 100);
...
int32_t es8311_sample_frequency_config(es8311_handle_t dev, int mclk_frequency, int sample_frequency)
{
    ...
    /* register 0x02 */
    es8311_read_reg(dev, ES8311_CLK_MANAGER_REG02, &regv);
    regv &= 0x07;
    ...
    es8311_write_reg(dev, ES8311_CLK_MANAGER_REG03, reg03);

    /* register 0x04 */
    es8311_write_reg(dev, ES8311_CLK_MANAGER_REG04, selected_coeff->dac_osr);

    /* register 0x05 */
    const uint8_t reg05 = ((selected_coeff->adc_div - 1) << 4) | (selected_coeff->dac_div - 1);
    es8311_write_reg(dev, ES8311_CLK_MANAGER_REG05, reg05);

    /* register 0x06 */
    es8311_read_reg(dev, ES8311_CLK_MANAGER_REG06, &regv);
    regv &= 0xE0;
    ...
    /* register 0x07 */
    es8311_read_reg(dev, ES8311_CLK_MANAGER_REG07, &regv);
    regv &= 0xC0;
    regv |= selected_coeff->lrck_h << 0;
    es8311_write_reg(dev, ES8311_CLK_MANAGER_REG07, regv);

    /* register 0x08 */
    es8311_write_reg(dev, ES8311_CLK_MANAGER_REG08, selected_coeff->lrck_l);
...
static int32_t es8311_clock_config(es8311_handle_t dev, const es8311_clock_config_t *const clk_cfg, es8311_resolution_t res)
{
    ...
    es8311_write_reg(dev, ES8311_CLK_MANAGER_REG01, reg01);

    es8311_read_reg(dev, ES8311_CLK_MANAGER_REG06, &reg06);
    ...
    es8311_write_reg(dev, ES8311_CLK_MANAGER_REG06, reg06);
...
static int32_t es8311_fmt_config(es8311_handle_t dev, const es8311_resolution_t res_in, const es8311_resolution_t res_out)
{
    ...
    printf("ES8311 in Slave mode and I2S format");
    // uint8_t reg00;
    // es8311_read_reg(dev, ES8311_RESET_REG00, &reg00);
    // reg00 &= 0xBF;
    // es8311_write_reg(dev, ES8311_RESET_REG00, reg00); // Slave serial port - default
    // mdelay(20);
    ...
    es8311_write_reg(dev, ES8311_SDPIN_REG09, reg09);
    es8311_write_reg(dev, ES8311_SDPOUT_REG0A, reg0a);
...
int32_t es8311_init(es8311_handle_t dev, const es8311_clock_config_t *const clk_cfg, const es8311_resolution_t res_in, const es8311_resolution_t res_out)
{
    ...
    es8311_write_reg(dev, ES8311_RESET_REG00, 0x80); // Power-on command

    /* Setup clock: source, polarity and clock dividers */
    es8311_clock_config(dev, clk_cfg, res_out);

    /* Setup audio format (fmt): master/slave, resolution, I2S */
    es8311_fmt_config(dev, res_in, res_out);
...
{
    uint8_t reg32;
    es8311_read_reg(dev, ES8311_DAC_REG32, &reg32);
...
{
    uint8_t reg31;
    es8311_read_reg(dev, ES8311_DAC_REG31, &reg31);
...
{
    uint8_t reg37;
    es8311_read_reg(dev, ES8311_DAC_REG37, &reg37);
    reg37 &= 0x0F;
    reg37 |= (fade << 4);
...
{
    uint8_t reg15;
    es8311_read_reg(dev, ES8311_ADC_REG15, &reg15);
    reg15 &= 0x0F;
    reg15 |= (fade << 4);
...
es8311_dev_t sensor;
es8311_handle_t es8311_create(const csi_iic_t *port, const uint16_t dev_addr)
{
    sensor.port = port;
