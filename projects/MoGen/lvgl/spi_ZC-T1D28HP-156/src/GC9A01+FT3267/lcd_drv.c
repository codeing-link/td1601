...
#else
    /* 批量发送：先做字节序转换，再一次性发出去，避免逐像素调用 SPI */
    #define LCD_LINE_BUF_PX  240
    uint8_t line_buf[LCD_LINE_BUF_PX * 2];
    uint32_t sent = 0;
    while (sent < len) {
        uint32_t chunk = len - sent;
        if (chunk > LCD_LINE_BUF_PX) chunk = LCD_LINE_BUF_PX;
        uint32_t i;
        for (i = 0; i < chunk; i++) {
            line_buf[i * 2]     = color[sent + i] >> 8;
            line_buf[i * 2 + 1] = color[sent + i] & 0xFF;
        }
        lcd_spi_send(line_buf, chunk * 2);
        sent += chunk;
    }
#endif
...
void lcd_send_pixels(const uint8_t *data, uint32_t size)
{
    LCD_DC_HIGH();
    LCD_CS_LOW();
    csi_spi_frame_len(&lcd_spi, SPI_FRAME_LEN_16);
    csi_spi_send(&lcd_spi, data, size, 5000);
    csi_spi_frame_len(&lcd_spi, SPI_FRAME_LEN_8);
    LCD_CS_HIGH();
}
