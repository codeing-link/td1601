...
static uint32_t csitimer_regs_saved[CONFIG_CORETIM_REGISTER_NUM_SAVE];
...
static void do_prepare_sleep_action(void)
{
    ...
    csitimer_regs_saved[1] = *((volatile uint32_t *)(ibase + 2U));
...
//pll stable counter:不同的芯片不一样的值，需要关注
    pbase->CSSCR = 0x40000;
    ...
    *((volatile uint32_t *)(time_ibase + 2U)) = csitimer_regs_saved[1];
...
__attribute__((section(".ram.code"))) void soc_sleep(lpm_mode_e mode)
{
    ...
    /* clear pend IRQ */
    //不能把pmu_wakeup中断号使能去掉，否则会导致cpu不能唤醒，浅睡眠会进入中断处理函数，所以把CPU中断关闭，不响应处理函数，唤醒之后开CPU中断
    uint8_t i = 0;
    for (i = 0; i < 64; i++) {
        CLIC->CLICINT[i].IE = 0;
		CLIC->CLICINT[i].IP = 0;
	...
	csi_irq_disable((uint32_t)WJ_PMU_WAKEUP_IRQn);
    __enable_irq();
#endif
}
