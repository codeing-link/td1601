...
static void ch_list_add(csi_dma_t *dma, csi_dma_ch_t *dma_ch)
{
    ...
    if (dma->ch_list == NULL) {
        ...
        while (tmp->next != NULL) {
            tmp = (csi_dma_ch_t *)(tmp->next);
        }

        (tmp->next) = (void *)dma_ch;
    }
}
...
static void ch_list_delete(csi_dma_t *dma, csi_dma_ch_t *dma_ch)
{
    ...
    if (dma->ch_list == dma_ch) {
        dma->ch_list = (void *)dma->ch_list->next;
    } else {
        tmp = dma->ch_list;
        tmp_next = (void *)dma->ch_list->next;
...
void dw_dma_clear_int_dma(dw_dma_regs_t *dma_addr, uint8_t ch_id, uint32_t  value)
{
    switch (ch_id) {
        case 0U:
            ...
            while (dw_dma_get_statussrctran_dma0(dma_addr) & value) {
                dw_dma_set_clearsrctran_dma0(dma_addr, value);
            }

            while (dw_dma_get_statusdsttran_dma0(dma_addr) & value) {
                dw_dma_set_cleardsttran_dma0(dma_addr, value);
            }

            while (dw_dma_get_statuserr_dma0(dma_addr) & value) {
                dw_dma_set_clearerr_dma0(dma_addr, value);
            }
            while (dw_dma_get_statustfr_dma0(dma_addr) & (value)) {
                dw_dma_set_cleartfr_dma0(dma_addr, value);
            }
            while (dw_dma_get_statusblock_dma0(dma_addr) & value) {
                dw_dma_set_clearblock_dma0(dma_addr, value);
            }
            break;

        case 1U:
            while (dw_dma_get_statustfr_dma1(dma_addr) & (value)) {
                dw_dma_set_cleartfr_dma1(dma_addr, value);
            }
            while (dw_dma_get_statusblock_dma1(dma_addr) & value) {
                dw_dma_set_clearblock_dma1(dma_addr, value);
            }

            while (dw_dma_get_statussrctran_dma1(dma_addr) & value) {
                dw_dma_set_clearsrctran_dma1(dma_addr, value);
            }

            while (dw_dma_get_statusdsttran_dma1(dma_addr) & value) {
                dw_dma_set_cleardsttran_dma1(dma_addr, value);
            }

            while (dw_dma_get_statuserr_dma1(dma_addr) & value) {
...
void dw_dma_irq_handler(void *arg)
{
    ...
    if (int_status != ~0U) {
        ...
        do {
            if ((1 << (dma_ch->ch_id)) & int_status) {
                if (dma_ch->callback != NULL) {
                    dw_dma_clear_int_dma(dma_addr, ch_index, 1 << (dma_ch->ch_id));
                    dma_ch->callback(dma_ch, cb_state, dma_ch->arg);
                }
            }

            dma_ch = (void *)dma_ch->next;
        } while (dma_ch != NULL);
    }
}
...
csi_error_t csi_dma_ch_attach_callback(csi_dma_ch_t *dma_ch, void *callback, void *arg)
{
    ...
    if (dma_ch->ctrl_id == 0U) {
        ...
        //open mask
        dw_dma_set_masktfr_dma0(dma_addr, (DW_DMA_MaskTfr_DMA0_En_CH0 << dma_ch->ch_id));
        dw_dma_set_maskblock_dma0(dma_addr, (DW_DMA_MaskBlock_DMA0_En_CH0 << dma_ch->ch_id));
        // dw_dma_set_masksrctran_dma0(dma_addr, (DW_DMA_MaskSrcTran_DMA0_En_CH0 << dma_ch->ch_id));
        // dw_dma_set_maskdsttran_dma0(dma_addr, (DW_DMA_MaskDstTran_DMA0_En_CH0 << dma_ch->ch_id));
        dw_dma_set_maskerr_dma0(dma_addr, (DW_DMA_MaskErr_DMA0_En_CH0 << dma_ch->ch_id));

        //clear interrupt flag
        dw_dma_clear_int_dma(dma_addr, 0U, (DW_DMA_MaskTfr_DMA0_En_CH0 << dma_ch->ch_id));

    } else if (dma_ch->ctrl_id == 1U) {
        dma_addr = (dw_dma_regs_t *)(dma_array[dma_ch->ctrl_id]->dev.reg_base);
        //open mask
        dw_dma_set_masktfr_dma1(dma_addr, (DW_DMA_MaskTfr_DMA1_En_CH0 << dma_ch->ch_id));
        dw_dma_set_maskblock_dma1(dma_addr, (DW_DMA_MaskBlock_DMA1_En_CH0 << dma_ch->ch_id));
        // dw_dma_set_masksrctran_dma1(dma_addr, (DW_DMA_MaskSrcTran_DMA1_En_CH0 << dma_ch->ch_id));
        // dw_dma_set_maskdsttran_dma1(dma_addr, (DW_DMA_MaskDstTran_DMA1_En_CH0 << dma_ch->ch_id));
        dw_dma_set_maskerr_dma1(dma_addr, (DW_DMA_MaskErr_DMA1_En_CH0 << dma_ch->ch_id));

        //clear interrupt flag
        dw_dma_clear_int_dma(dma_addr, 1U, (DW_DMA_MaskTfr_DMA1_En_CH0 << dma_ch->ch_id));
...
void csi_dma_ch_detach_callback(csi_dma_ch_t *dma_ch)
{
    ...
    if (dma_ch->ctrl_id == 0U) {
        dma_addr = (dw_dma_regs_t *)(dma_array[dma_ch->ctrl_id]->dev.reg_base);
        //open mask
        dw_dma_reset_masktfr_dma0(dma_addr, (DW_DMA_MaskTfr_DMA0_En_CH0 << dma_ch->ch_id));
        dw_dma_reset_maskblock_dma0(dma_addr, (DW_DMA_MaskBlock_DMA0_En_CH0 << dma_ch->ch_id));
        dw_dma_reset_maskerr_dma0(dma_addr, (DW_DMA_MaskErr_DMA0_En_CH0 << dma_ch->ch_id));
        ...
        dma_addr = (dw_dma_regs_t *)(dma_array[dma_ch->ctrl_id]->dev.reg_base);
        //open mask
        dw_dma_reset_masktfr_dma1(dma_addr, (DW_DMA_MaskTfr_DMA1_En_CH0 << dma_ch->ch_id));
        dw_dma_reset_maskblock_dma1(dma_addr, (DW_DMA_MaskBlock_DMA1_En_CH0 << dma_ch->ch_id));
        dw_dma_reset_maskerr_dma1(dma_addr, (DW_DMA_MaskErr_DMA1_En_CH0 << dma_ch->ch_id));
