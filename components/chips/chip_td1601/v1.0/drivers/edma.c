...
extern uint16_t dma_ch_tfrdone_hs_num[];
extern uint16_t dma_ch_evtdone_hs_num[];
static csi_edma_t *dma_array[4];
            ...
            if (dma_ch != NULL) {
                if (int_status & WJ_DMA_CH_INTS_ERR) {
                    ...
                    if (dma_ch->callback != NULL) {
                        dma_ch->callback(dma_ch, EDMA_EVENT_TRANSFER_ERROR, dma_ch->arg);
                    }
                }
                ...
                if (int_status & WJ_DMA_CH_INTS_HTFR) {
                    ...
                    if (dma_ch->callback != NULL) {
                        dma_ch->callback(dma_ch, EDMA_EVENT_TRANSFER_HALF_DONE, dma_ch->arg);
                    }
                }
                ...
                if (int_status & WJ_DMA_CH_INTS_TFR) {
                    ...
                    if (dma_ch->callback != NULL) {
                        dma_ch->callback(dma_ch, EDMA_EVENT_TRANSFER_DONE, dma_ch->arg);
                    }
                }
            }
        }
...
void csi_edma_ch_free(csi_edma_ch_t *dma_ch)
{
    ...
    if (dma_ch->dma_ll_ch) {
        ch_alloc_status = 1U << (uint8_t)(dma_ch->dma_ll_ch->ch_id);

        if (dma_array[dma_ch->dma_ll_ch->ctrl_id]->alloc_status & ch_alloc_status) {
            dma_array[dma_ch->dma_ll_ch->ctrl_id]->alloc_status &= ~ch_alloc_status;
            slist_del(&dma_ch->dma_ll_ch->next, &dma_array[dma_ch->dma_ll_ch->ctrl_id]->head);

            if (dma_ch->dma_ll_ch->etb_ch_id != -1) {
                csi_etb_ch_free((int32_t)(dma_ch->dma_ll_ch->etb_ch_id));
            }
        }
    }
...
csi_error_t csi_edma_ch_config(csi_edma_ch_t *dma_ch, csi_edma_ch_config_t *config)
{
    ...
    if (config->trans_dir == EDMA_MEM2MEM) {
    ...
    if (!config->link_list_en) {
        /* etb config */
        if ((dma_ch->etb_ch_id == -1) && (config->trans_dir != EDMA_MEM2MEM)) {
        ...
        ret = csi_edma_ch_alloc(dma_ch->dma_ll_ch, -1, -1);
        if (ret == CSI_OK) {
            int32_t etb_ll_ch;
            csi_edma_ch_t *dma_ll_ch = dma_ch->dma_ll_ch;
            wj_dma_ch_regs_t *dma_ch_addr = (wj_dma_ch_regs_t *)(((dma_ll_ch->ch_id) * 0x30U) + dma_array[dma_ll_ch->ctrl_id]->dev.reg_base);

            /* Initializes corresponding channel registers */
            wj_dma_set_transferwidth(dma_ch_addr, EDMA_DATA_WIDTH_32_BITS, EDMA_DATA_WIDTH_32_BITS);
            wj_dma_set_addrinc(dma_ch_addr, EDMA_ADDR_INC, EDMA_ADDR_INC);
            /* set little endian */
            wj_dma_set_addr_endian(dma_ch_addr, 0U, 0U);

            /* set group mode */
            wj_dma_trans_mode_set(dma_ch_addr, WJ_DMA_CH_CTRLB_TRGTMDC_GROUP_TRIG_MODE);
            wj_dma_set_groupmode(dma_ch_addr, EDMA_ADDR_INC, EDMA_ADDR_INC, 0U, sizeof(wj_dma_ch_regs_t));
            ...
            wj_dma_set_channel_addr(dma_ch_addr, (uint32_t)soc_dma_address_remap((unsigned long)config->link_list_config.lli_buf), (uint32_t)soc_dma_address_remap((unsigned long)temp));
            dma_set_int_mask(dma_ch_addr, WJ_DMA_CH_INTM_ERR);
            ...
            if (dma_ll_ch->etb_ch_id == -1) {
                etb_ll_ch = csi_etb_ch_alloc(ETB_CH_ONE_TRIGGER_ONE);

                if (etb_ll_ch == CSI_ERROR) {
                    ret = CSI_ERROR;
                }

                dma_ll_ch->etb_ch_id = (int16_t)etb_ll_ch;
            }

            etb_config.src_ip = (uint8_t)dma_ch_tfrdone_hs_num[dma_ch->ch_id];
            etb_config.dst_ip = (uint8_t)dma_ll_ch->ch_id;
            etb_config.ch_type = ETB_CH_ONE_TRIGGER_ONE;
            etb_config.trig_mode = ETB_HARDWARE_TRIG;
            ret = csi_etb_ch_config((int32_t)(dma_ch->etb_ch_id), &etb_config);

            etb_config.src_ip = (uint8_t)dma_ch_evtdone_hs_num[dma_ll_ch->ch_id];
            etb_config.dst_ip = (uint8_t)dma_ch->ch_id;
            etb_config.ch_type = ETB_CH_ONE_TRIGGER_ONE;
            etb_config.trig_mode = ETB_HARDWARE_TRIG;
            ret = csi_etb_ch_config((int32_t)(dma_ll_ch->etb_ch_id), &etb_config);

            csi_etb_ch_start((int32_t)(dma_ll_ch->etb_ch_id));
        }
    }
    return ret;
}
...
csi_error_t csi_edma_add_link_list_item(csi_edma_ch_t *dma_ch, csi_edma_ch_config_t *config, csi_edma_link_list_item_t *item, uint8_t is_last_item)
{
    ...
    uint8_t *temp_addr;
    wj_dma_ch_regs_t *dma_ch_addr;
    temp_addr = (uint8_t *)config->link_list_config.lli_buf + sizeof(wj_dma_ch_regs_t) * config->link_list_config.link_list_num;
    dma_ch_addr = (wj_dma_ch_regs_t *)temp_addr;
    ...
    if (judge_valid(dma_width_to_bytes, dma_width_to_bytes, 0, length)) {
        return CSI_ERROR;
    }
    /* Initializes corresponding channel registers */
    wj_dma_set_transferwidth(dma_ch_addr, (uint32_t)(config->src_tw), (uint32_t)(config->dst_tw));
    ...
    wj_dma_set_total_size(dma_ch_addr, length);
    wj_dma_set_channel_addr(dma_ch_addr, (uint32_t)soc_dma_address_remap((unsigned long)item->srcaddr), (uint32_t)soc_dma_address_remap((unsigned long)item->dstaddr));
    ...
    if (is_last_item) {
        ...
        dma_set_ch_int_en(dma_ch_addr, WJ_DMA_CH_CTRLB_INT_EN);
        uint32_t temp;
        temp = ((uint32_t)(dma_ch->dma_ll_ch->ch_id) * 0x30U) + dma_array[dma_ch->dma_ll_ch->ctrl_id]->dev.reg_base;
        dma_ch_addr = (wj_dma_ch_regs_t *)(temp);
        wj_dma_set_total_size(dma_ch_addr, sizeof(wj_dma_ch_regs_t) * config->link_list_config.link_list_num);
        dma_set_ch_en(dma_ch_addr, WJ_DMA_CH_EN);
    }
    soc_dcache_clean_invalid_range((unsigned long)(config->link_list_config.lli_buf), sizeof(wj_dma_ch_regs_t) *config->link_list_config.link_list_num);
    return ret;
}
