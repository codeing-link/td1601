...
def makefile(project: Package, packages: list[Package], board: Package, sources: list[tuple[str, Path]], includes: list[Path], defines: dict, cflags: str, ldflags: str, libs: list[str]) -> str:
    ...
    for label, source in sources:
        stem = re.sub(r"[^A-Za-z0-9_]", "_", f"{label}_{source.name}")
        name = stem
        n = 2
        while name in used:
            name = f"{stem}_{n}"; n += 1
        used.add(name)
        obj_entries.append((f"$(OBJ)/{name}.o", source))
    ...
    lines.append("INCLUDES := " + " ".join(f"-I{p}" for p in includes))
...
def main() -> None:
    ...
    parser.add_argument("--force", action="store_true", help="overwrite generated Makefile, packaging, and burn scripts")
    ...
    outputs = [root / "Makefile", root / "aft_build_macos.sh", root / "build_patch_download.sh"]
